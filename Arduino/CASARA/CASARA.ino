...
int myvar[ADC_CHANNELS];              // Create anarray to store the results of the ADC conversions
...
// Processor loop
void loop(){
  if (readFlag == 1){                  // Check to see if the value has been updated from ADC
...
void clipping () {
  if (myvar[2] > maximum) {            
    maximum = myvar[2];                // Set maximum value from pin A1
  }
